...
impl PhysicsHooks for RawPhysicsHooks {
    ...
    fn modify_solver_contacts(&self, ctxt: &mut ContactModificationContext) {
        ...
        let point1 =  ctxt.solver_contacts.get(0).map_or(default_point, |contact| contact.point);
        let p1x = &point1.x;
        let p1y = &point1.y;
        let p1 = &combine_f32_to_f64(*p1x, *p1y);


        let point2 =  ctxt.solver_contacts.get(1).map_or(default_point, |contact| contact.point);
        let p2x = &point2.x;
        let p2y = &point2.y;
        let p2 = &combine_f32_to_f64(*p2x, *p2y);


        let ret = self.modify_solver_contacts
            .bind2(
                &self.this,
                &JsValue::from(utils::flat_handle(ctxt.collider1.0)),
                &JsValue::from(utils::flat_handle(ctxt.collider2.0)),
